...
def hide_message(img, bin_text, bit_planes):
    ...
    for plane_idx in range(num_planes):
        ...
        text_shifted_bitwise = np.concatenate((text_shifted_bitwise, np.zeros(color_planes - len(text_shifted_bitwise), dtype=np.uint8)))
...
bit_planes = np.array(list(sys.argv[3]), dtype=np.uint8)[::-1]
